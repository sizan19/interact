...
local selected, unselected, interact, pin = settings.Textures.selected, settings.Textures.unselected, settings.Textures.interact1, settings.Textures.pin
        ...
        if CheckCanInteract(interaction) then
            ...
            if GetScreenCoordFromWorldCoord(coords.x, coords.y, coords.z) then
                if i == 1 and (interaction.curDist <= interaction.interactDst) and (interaction.entity and {(interaction.entity == CurrentTarget)} or {true})[1] then
                    ...
                    SetScriptGfxAlignParams(0.004, -0.002, 0.0, 0.0)
                    SetDrawOrigin(coords.x, coords.y, coords.z)
                    DrawSprite('interactions_txd', interact, 0, 0, 0.0185, 0.0333333333333333, 0, 255, 255, 255, 255)
                    ResetScriptGfxAlign()
                    ...
                    if table_type(options) ~= 'empty' then
                        ...
                        if CURRENT_SELECTION ~= 1 and (IsControlJustPressed(0, 172) or IsControlJustPressed(0, 35)) then
                            CURRENT_SELECTION -= 1
                    ...
                    DrawSprite('interactions_txd', pin, 0, 0, 0.015, 0.030, 0, 255, 255, 255, 255)
                end
